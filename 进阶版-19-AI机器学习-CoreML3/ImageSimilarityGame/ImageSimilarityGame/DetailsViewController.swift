...
class DetailsViewController: UICollectionViewController {
    ...
    override func collectionView(_ collectionView: UICollectionView,
        ...
        let view = collectionView.dequeueReusableSupplementaryView(ofKind: DetailsLayout.backgroundViewKind, withReuseIdentifier: DetailsBackgroundView.reuseIdentifier, for: indexPath)
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        var title = "详情"
        var prompt = "点击学生的临摹画查看与原画的distance值"
        if indexPath.item > 0 {
            let node = nodes[indexPath.item]
            title = node.label
            prompt = "Distance值: \(node.distance)"
        }
